...
object  ModelScoreActor {

  case class BuildModelScore(user_id: String, dataset_path: String, scored_dataset_id: String,
                               meta:ModelScoreJob, job_id: Long)
...
class ModelScoreActor @Inject()(s3Config: S3Config, jobResult: JobResult,
                                  filesystemService: FilesystemService,
                                  configuration: Configuration,
                                  fileMetadataDAO: FileMetadataDAO) extends Actor {

  import ModelScoreActor._
  ...
  def receive = {
    case BuildModelScore(user_id: String, dataset_path: String, scored_dataset_id: String,
    meta:ModelScoreJob, job_id: Long) => {
      ...
      val new_model_datapath = getFolderName(user_id)+ "/score/"+ unique_id +".parquet"


      val result = saveRequestToS3(request_filepath, Json.toJson(meta))

      checkInS3(request_filepath)
      val job_status = run_spark_submit_command(dataset_path_parquet,
        request_filepath, new_model_datapath)
      jobResult.updateJobStatus(job_id, job_status) map { res =>
        if(job_status == 0) {
          jobResult.saveModelScore(new_model_datapath, scored_dataset_id, meta) map { result =>
            context.stop(self)
          }
        }else
          context.stop(self)

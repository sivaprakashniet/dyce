...
import entities.{ModelMeta, RandomForestMeta}
...
object  RandomForestActor {
  ...
  case class BuildRandomForest(user_id: String, tree_id: Option[String], dataset_path: String,
                               rf_meta: RandomForestMeta, job_id: Long)
...
class RandomForestActor @Inject()(s3Config: S3Config, jobResult: JobResult,
                            filesystemService: FilesystemService,
                            configuration: Configuration,
                            fileMetadataDAO: FileMetadataDAO) extends Actor {

  import RandomForestActor._
  ...
  def receive = {
    case BuildRandomForest(user_id: String, tree_id: Option[String],
                        dataset_path: String, rf_meta: RandomForestMeta, job_id: Long) => {
      ...
      val result = saveRequestToS3(request_filepath, Json.toJson(rf_meta))
      ...
      jobResult.updateJobStatus(job_id, job_status) map { res =>
        if(job_status == 0) {
          jobResult.saveRandomForestFromS3(response_filepath, tree_id, rf_meta.dataset_id) map (res => {
            val new_model = new ModelMeta(Some(unique_id),
              rf_meta.name,
              rf_meta.dataset_id,
              res._2,
              user_id,
              rf_meta.dataset_name,
              res._1,
              rf_meta.input_column.mkString(","),
              rf_meta.output_column,
              model_path, None)
